...
#Results:
    #Table XXX show that for 9 species, at least one model has a Boyce index clearly above 0, i.e., working better than the random expectation. 4 of these species show a high Boyce index (above 0.5), indicating a good ability to predict naturalized occurrences from unseen data. Note that, from the original 25 species of Perret's dataset, we could only perform the complete evaluation of the models for 14 species due to a reduced number of presences after data cleaning. Therefore, our models had a better performance predicting new occurrences than the random expectation for more than half of the species analyzed. This suggests that our models could be useful to predict suitability of at least some pines species, but it is important to bear in mind that we did not perform an exhaustive independent evaluation of the whole Pinus genus as we were bounded by the availability of naturalized occurrences.
