...
plot_suples = function(species, size="big"){
    ...
    current_suit = raster(paste("/Volumes/Maxtor/diego/science_big_documents/pines_niche/ensambles_final/ensamble_predictions_bin/ensamble_predictions_bin_", species, ".tif", sep=""))
    ...
    projected_suit = raster(paste("/Volumes/Maxtor/diego/science_big_documents/pines_niche/ensambles_final/ensamble_projections_bin/ensamble_projections_bin_", species, ".tif", sep=""))

    #crop projected suitability
    projected_suit = crop(projected_suit, polygon_plot_buffer)

    #load ocurrences
    ocurrence_data = read.csv(paste("/Volumes/Maxtor/diego/science_big_documents/pines_niche/ocurrences/", species, "_complete.presences.csv", sep=""), header=TRUE)
    ...
    median_metrics = read.table("/Volumes/GoogleDrive/My Drive/science/phd/nicho_pinus/results/evaluations/medians_evaluations_v2.csv", sep=",", header=TRUE)
    ...
    legend(xmin(current_suit), ymin(current_suit),legend=bquote(bold('RF: AUC') == .(rf_auc_median) %+-% .(rf_auc_sd) ~ bold('Kappa') == .(rf_kappa_median) %+-% .(rf_kappa_sd) ~ bold('TSS') == .(rf_tss_median) %+-% .(rf_tss_sd)),cex=.7, text.col="black", box.col="black",bg="white", adj=c(0.035,0.5), xjust=0, yjust=-2)    
...
plot_current_main = function(species, legend=TRUE, small=FALSE, size="big", title_species=TRUE){
    ...
    ##plot the results using legend, because let us to include a background
    legend(xmin(current_suit), ymin(current_suit),legend=bquote(bold('RF: AUC') == .(rf_auc_median) %+-% .(rf_auc_sd) ~ bold('Kappa') == .(rf_kappa_median) %+-% .(rf_kappa_sd) ~ bold('TSS') == .(rf_tss_median) %+-% .(rf_tss_sd)),cex=0.95, text.col="black", box.col="black",bg="white", adj=c(0.035,0.5), xjust=0, yjust=-2, x.intersp=0.42)  
...
########################################
##### Figures for PhD dissertation #####
########################################
pdf("/Volumes/GoogleDrive/My Drive/science/phd/defensa/presentacion_defensa_v3/fotos/projected_suitability_with_proportions_iberian_pines.pdf", width=12, height = 12)
par(oma=c(0,0,2.7,2))
par(mfrow=c(2,2))
plot_proj_main(species="halepensis", proportions=TRUE, legend=TRUE)
plot_proj_main(species="nigra", size="medium", proportions=TRUE, legend=FALSE)
plot_proj_main(species="sylvestris", proportions=TRUE, legend=FALSE)
plot_proj_main(species="mugo", proportions=TRUE, legend=FALSE)
dev.off()