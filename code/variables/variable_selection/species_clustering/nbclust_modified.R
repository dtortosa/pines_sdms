#Nbclust function modified. The rationale of use this modified function that I obtain an error with complete method: "Error in solve.default(W) : system is computationally singular: reciprocal condition number = 1.12144e-16". 
    ...
    max.nc = 15, method = NULL, index = "all", alphaBeale = 0.1) 
{
    ...
    Indices.WBT <- function(x, cl, P, s, vv) {
        ...
        friedman <- sum(diag(solve(W, tol=1e-19) * B))
