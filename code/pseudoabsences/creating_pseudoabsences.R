...
length(global_strata) #number of categories (36)
...
pseudo_absence = function(i){
    ...
    ###Create the pseudoabsece buffer for crop the environmental variables. The difference between this buffer and the distribution_buffer will be the area with PAs.
    #create a polygon buffer around the distribution buffer
    polygon_PA_buffer = gBuffer(polygon_buffer, byid=FALSE, id=NULL, width=50, quadsegs=5, capStyle="ROUND", joinStyle="ROUND", mitreLimit=1.0) 
    ...
    bio6_crop =  crop(bio6, raster_PA_buffer)
    bio18_crop =  crop(bio18, raster_PA_buffer) 
    clay_crop = crop(clay, raster_PA_buffer) #we use the raster of the PA buffer to crop the rest of variables because y faster than using a polygon. 
    ...
    if(5*nrow(data)/length(strata)<30){ #if for each class there is less of 30 points with a number of PAs of (5*ocurrences): 

        number_PAs = length(strata)*30 #with this number of PAs we have 30 PAs for each strata. 

    } else { 
        number_PAs = 5*nrow(data)
    }
    ...
    png(paste("/Volumes/GoogleDrive/My Drive/science/phd/nicho_pinus/results/pseudo_absences", paste(i, "pseudo_absences_types_samplings_without_resampling.png", sep="_"),sep="/"), width=1300, height=1300)
    par(mfcol=c(2,2))
    plot(variables_categories, col=yb, main="Equal Sampling")
    points(envstrat_ecu$x,envstrat_ecu$y,pch=16,cex=.5,col="black")
    plot(variables_categories, col=yb, main="Proportional Sampling")
    points(envstrat_prp$x,envstrat_prp$y,pch=16,cex=.5,col="black")
    ...
    points(envstrat_prp[c(points_outside_PA_buffer),]$x, envstrat_prp[c(points_outside_PA_buffer),]$y, pch=16,cex=.5,col="yellow") #points outside the PA buffer
    points(envstrat_prp[points_inside_distribution_buffer,]$x, envstrat_prp[points_inside_distribution_buffer,]$y,pch=16,cex=.5,col="blue") #points inside distribution buffer
    points(envstrat_prp[-c(points_outside_PA_buffer, points_inside_distribution_buffer),]$x,envstrat_prp[-c(points_outside_PA_buffer, points_inside_distribution_buffer),]$y, pch=16,cex=.5,col="black") #these points are the interesting for me. 
    ...
    PA$precision_weight = rep(x=sum(data$precision_weight)/nrow(PA), #repeat the sum of all weights of presences divided by the number of presences
    ...
    nrow(final) == number_PAs #the number of PAs is 5 times the number of presences
    ...
    #plot
    png(paste("/Volumes/GoogleDrive/My Drive/science/phd/nicho_pinus/results/pseudo_absences", paste(i, "pseudo_absences_on_classes.png", sep="_"),sep="/"), width=1300, height=1300)
    par(mfcol=c(1,2))
    plot(variables_categories, col=yb, main="Without resampling")
    points(PA$longitude,PA$latitude,pch=16,cex=.5,col="black")
    plot(variables_categories, col=yb, main="With resampling")
    points(PA_final$longitude,PA_final$latitude,pch=16,cex=.5,col="black")
    dev.off()
    ...
    #plot all 
    png(paste("/Volumes/GoogleDrive/My Drive/science/phd/nicho_pinus/results/pseudo_absences", paste(i, "PA_presences.png", sep="_"),sep="/"), width=1300, height=1300)
    plot(bio6_crop, col="gray80")
    points(complete.presences[complete.presences$presence==1,]$longitude, complete.presences[complete.presences$presence==1,]$latitude, cex=.5, col="red", pch=16)
    points(complete.presences[complete.presences$presence==0,]$longitude, complete.presences[complete.presences$presence==0,]$latitude, cex=.5, col="blue", pch=16)
