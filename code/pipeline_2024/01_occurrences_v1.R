...
if(FALSE){
    ...
    writeRaster(raster_buffer, paste("./results/pipeline_2024/", species, "/occurrences/ocurrences_", species, "_distribution_buffer.asc", sep=""), format="ascii", overwrite=TRUE)
        ...
        #------------------------------
        #importa la tabla
        presencia.completa<-read.csv(paste("./datos/raw_ocurrences/ocurrences/tree_species/final_species/Pinus", paste(i, "csv", sep="."), sep="_"), header=T, fill=TRUE, check.names=TRUE, stringsAsFactors=FALSE) #stringsAsFactors=FALSE sirve para procesar mejor la columna coordinatePrecision, que lleva mezclados números y caracteres
        ...
        #plot the result
        png(paste("./results/ocurrences", paste(i, "ocurrences_inside_buffer.png", sep="_"), sep="/"), width=1500, height=800, pointsize=30)
        ...
        #open png
        png(paste("./results/ocurrences/", i, "_check_cell_number.png", sep=""), width=1100, height=800, pointsize=30)
        ...
        if(nrow(final.presences[which(final.presences$precision_weight==1),]) > 1){
            ...
            #open png
            png(paste("./results/ocurrences/", i, "_check_altitudinal_sampling.png", sep=""), width=1100, height=800, pointsize=30)
        ...
        #ploteamos 
        pdf(paste("./results/ocurrences", paste(i, "bianca_cells_with_without_gbif_points.pdf", sep="_"), sep="/"), width=24, height=24)
        par(mfcol=c(2,2))
        ...
        #plot
        pdf(paste("./results/ocurrences", paste(i, "final_presences.pdf", sep="_"), sep="/"), width=12, height=12)
        plot(environment_crop, col="gray80")
        points(ultimate_ocurrences$longitude, ultimate_ocurrences$latitude,  cex=0.5, col="red")
        dev.off()

        #write the resulting data.frame in a csv. 
        write.csv(ultimate_ocurrences, paste("./results/ocurrences", paste(i, "final.presences.csv", sep="_"), sep="/"), row.names=FALSE)

        save.image(paste("./results/rdata", paste(i, "preparation_presences.RData", sep="_"), sep="/"))
    }
}